...
However, Figure~\ref{fig:baseline_cm} shows that several recordings of \textit{Brachypteryx montana} and \textit{Dicrurus balicassius} were misclassified as \textit{Hypsipetes philippinus}, suggesting that the global recording-level representation was not always sufficient to captureaco ustic differences between some bird vocalizations.
\subsection{Tuned Baseline Results}
The final held-out test set contained 71 recordings. The baseline model correctly classified 53 recordings and misclassified 18, producing an accuracy of 0.72 and a macro F1-score of 0.71. Table~\ref{tab:tuned_baseline_classification_report} presents the per-class results.
